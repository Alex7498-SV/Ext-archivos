...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bt_write_external.setOnClickListener {
            val ext_route = Environment.getExternalStorageDirectory()
            val file = "email.txt"
            val fi = File(ext_route.path, file)

            //TODO Logs de prueba para verificar el estado del almacenamiento
            Log.d("ruta", "${Environment.getExternalStorageDirectory()}")
            Log.d("ruta", "${fi}")

            //TODO Codigo Semi funcional

            try {
                val fileOutputStream = OutputStreamWriter(FileOutputStream(fi))
                fileOutputStream.write(et_option.text.toString())
    ...
    override fun onStart() {
        ...
        if(ContextCompat.checkSelfPermission(this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED){
            ...
            ActivityCompat.requestPermissions(this, arrayOf(android.Manifest.permission.WRITE_EXTERNAL_STORAGE), REQUEST)
        }
    }
}
